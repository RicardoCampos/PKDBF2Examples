namespace System.Security.Cryptography
{
    ...
    /// <remarks>
    /// Drop in replacement for Rfc2898DeriveBytes but using SHA256
    /// using some non-internal classes
    /// </remarks>

    [Runtime.InteropServices.ComVisible(true)]
    public class Rfc2898DeriveBytes_HMACSHA256 : DeriveBytes
    {
        private byte[] _buffer;
        private byte[] _salt;
        private readonly HMACSHA256 _hmacsha256;  // The pseudo-random generator function used in PBKDF2
        ...
        public Rfc2898DeriveBytes_HMACSHA256(string password, int saltSize) : this(password, saltSize, 1000) { }
        [SecuritySafeCritical]
        public Rfc2898DeriveBytes_HMACSHA256(string password, int saltSize, int iterations)
        {
            if (saltSize < 0)
            ...
            _hmacsha256 = new HMACSHA256(new UTF8Encoding(false).GetBytes(password));
            Initialize();
        }

        public Rfc2898DeriveBytes_HMACSHA256(string password, byte[] salt) : this(password, salt, 1000) { }

        public Rfc2898DeriveBytes_HMACSHA256(string password, byte[] salt, int iterations) : this(new UTF8Encoding(false).GetBytes(password), salt, iterations) { }
        ...
        private byte[] Func()
        {
            ...
            _hmacsha256.TransformBlock(_salt, 0, _salt.Length, _salt, 0);
            _hmacsha256.TransformFinalBlock(intBlock, 0, intBlock.Length);
            var temp = _hmacsha256.Hash;
            _hmacsha256.Initialize();

            var ret = temp;
            for (var i = 2; i <= _iterations; i++)
            {
                temp = _hmacsha256.ComputeHash(temp);
